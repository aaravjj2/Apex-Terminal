/**
 * CrossMarginUI2 — W60: Cross-Margin
...
export function CrossMarginUI2() {
  ...
  const kpiItems: KPIItem[] = [
    ...
    { id: 'week', label: 'Week', value: 'W60', status: 'neutral', icon: <span style={{ fontSize: '16px' }}>💼</span> },
    { id: 'version', label: 'API Version', value: 'v4', status: 'neutral', icon: <span style={{ fontSize: '16px' }}>🔗</span> },
  ];

  return (
    <div data-testid="cross-margin-page" data-ready="true" style={S.page}>
      <div style={{ padding: '12px 16px 0 16px' }}>
        <PageHeader title="Cross-Margin" subtitle="Cross-margin controls with portfolio margining and collateral optimization" testId="cross-margin-header" />
      </div>
      <div style={{ padding: '0 16px 8px 16px' }}>
        <Tabs items={TABS} activeTab={tab} onTabChange={setTab} testId="cross-margin-tabs" />
      </div>
      <div style={S.content}>
        ...
        {tab === 'overview' && (
          <div style={S.gap}>
            <KPIStrip items={kpiItems} variant="hero" testId="cross-margin-kpi" />
            <div style={S.grid2}>
              <Panel title="Service Status" variant="elevated" padding="md" testId="cross-margin-status"
                status={<StatusBadge variant="success">Operational</StatusBadge>}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  <InfoRow label="Feature" value="Cross-Margin" />
                  <InfoRow label="Week" value="W60" />
                  <InfoRow label="API Prefix" value="/api/v4/cross-margin" />
                  <InfoRow label="Endpoints" value="5" />
                  <InfoRow label="Version" value="v4" />
                </div>
              </Panel>
              <Panel title="Quick Actions" variant="elevated" padding="md" testId="cross-margin-actions">
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--ui2-space-2)' }}>
                  <Button variant="primary" fullWidth onClick={fetchData} loading={loading} testId="cross-margin-refresh">
                    Refresh Data
                  </Button>
                  <Button variant="secondary" fullWidth testId="cross-margin-export">
                    Export Report
                  </Button>
                  <Button variant="ghost" fullWidth testId="cross-margin-docs">
                    View Documentation
                  </Button>
                </div>
              </Panel>
        ...
        {tab === 'data' && (
          <div style={S.gap}>
            {loading ? <Skeleton height={300} /> : data.length > 0 ? (
              <Panel title="Cross-Margin Data" variant="default" padding="none" testId="cross-margin-data-panel">
                <DataTable columns={columns} data={data} keyField="id" density="compact" testId="cross-margin-table" />
              </Panel>
            ) : (
        ...
        {tab === 'analytics' && (
          <div style={S.gap}>
            <Panel title="Analytics Overview" variant="elevated" padding="md" testId="cross-margin-analytics">
              <div style={S.grid3}>
            ...
            <Panel title="Recent Activity" variant="bordered" padding="md" testId="cross-margin-activity">
        ...
        {tab === 'config' && (
          <div style={S.gap}>
            <Panel title="Service Configuration" variant="elevated" padding="md" testId="cross-margin-config">
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                <InfoRow label="API Endpoint" value="/api/v4/cross-margin" />
                <InfoRow label="Week" value="W60" />
            ...
            <Panel title="Endpoints" variant="bordered" padding="md" testId="cross-margin-endpoints">
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <div style={S.surface}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>Get margin requirements</div>
                </div>
                <div style={S.surface}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ui2-text-primary)' }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>Get collateral status</div>
                </div>
                <div style={S.surface}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ui2-text-primary)' }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>Optimize collateral</div>
                </div>
                <div style={S.surface}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ui2-text-primary)' }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>Get margin alerts</div>
                </div>
                <div style={S.surface}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ui2-text-primary)' }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>What-if margin analysis</div>
                </div>
              </div>
            </Panel>
          </div>
