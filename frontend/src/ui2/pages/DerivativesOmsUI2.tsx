/**
 * DerivativesOmsUI2 — W61: Derivatives OMS
...
const API = '/api/v4/derivatives-oms';
...
export function DerivativesOmsUI2() {
  ...
  const kpiItems: KPIItem[] = [
    ...
    { id: 'week', label: 'Week', value: 'W61', status: 'neutral', icon: <span style={{ fontSize: '16px' }}>🏢</span> },
    { id: 'version', label: 'API Version', value: 'v4', status: 'neutral', icon: <span style={{ fontSize: '16px' }}>🔗</span> },
  ];

  return (
    <div data-testid="derivatives-oms-page" data-ready="true" style={S.page}>
      <div style={{ padding: '12px 16px 0 16px' }}>
        <PageHeader title="Derivatives OMS" subtitle="Derivatives order management with multi-leg support and exercise management" testId="derivatives-oms-header" />
      </div>
      <div style={{ padding: '0 16px 8px 16px' }}>
        <Tabs items={TABS} activeTab={tab} onTabChange={setTab} testId="derivatives-oms-tabs" />
      </div>
      <div style={S.content}>
        ...
        {tab === 'overview' && (
          <div style={S.gap}>
            <KPIStrip items={kpiItems} variant="hero" testId="derivatives-oms-kpi" />
            <div style={S.grid2}>
              <Panel title="Service Status" variant="elevated" padding="md" testId="derivatives-oms-status"
                status={<StatusBadge variant="success">Operational</StatusBadge>}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  <InfoRow label="Feature" value="Derivatives OMS" />
                  <InfoRow label="Week" value="W61" />
                  <InfoRow label="API Prefix" value="/api/v4/derivatives-oms" />
                  <InfoRow label="Endpoints" value="5" />
                  <InfoRow label="Version" value="v4" />
                </div>
              </Panel>
              <Panel title="Quick Actions" variant="elevated" padding="md" testId="derivatives-oms-actions">
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--ui2-space-2)' }}>
                  <Button variant="primary" fullWidth onClick={fetchData} loading={loading} testId="derivatives-oms-refresh">
                    Refresh Data
                  </Button>
                  <Button variant="secondary" fullWidth testId="derivatives-oms-export">
                    Export Report
                  </Button>
                  <Button variant="ghost" fullWidth testId="derivatives-oms-docs">
                    View Documentation
                  </Button>
                </div>
              </Panel>
        ...
        {tab === 'data' && (
          <div style={S.gap}>
            {loading ? <Skeleton height={300} /> : data.length > 0 ? (
              <Panel title="Derivatives OMS Data" variant="default" padding="none" testId="derivatives-oms-data-panel">
                <DataTable columns={columns} data={data} keyField="id" density="compact" testId="derivatives-oms-table" />
              </Panel>
            ) : (
        ...
        {tab === 'analytics' && (
          <div style={S.gap}>
            <Panel title="Analytics Overview" variant="elevated" padding="md" testId="derivatives-oms-analytics">
              <div style={S.grid3}>
            ...
            <Panel title="Recent Activity" variant="bordered" padding="md" testId="derivatives-oms-activity">
        ...
        {tab === 'config' && (
          <div style={S.gap}>
            <Panel title="Service Configuration" variant="elevated" padding="md" testId="derivatives-oms-config">
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                <InfoRow label="API Endpoint" value="/api/v4/derivatives-oms" />
                <InfoRow label="Week" value="W61" />
            ...
            <Panel title="Endpoints" variant="bordered" padding="md" testId="derivatives-oms-endpoints">
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <div style={S.surface}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>List derivatives orders</div>
                </div>
                <div style={S.surface}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ui2-text-primary)' }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>Create derivatives order</div>
                </div>
                <div style={S.surface}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ui2-text-primary)' }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>List derivatives positions</div>
                </div>
                <div style={S.surface}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ui2-text-primary)' }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>Exercise option position</div>
                </div>
                <div style={S.surface}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ui2-text-primary)' }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>List expiring positions</div>
                </div>
              </div>
            </Panel>
          </div>
