/**
 * FactorModelUI2 — W32: Factor Model
...
export function FactorModelUI2() {
  ...
  const kpiItems: KPIItem[] = [
    ...
    { id: 'week', label: 'Week', value: 'W32', status: 'neutral', icon: <span style={{ fontSize: '16px' }}>🧮</span> },
    { id: 'version', label: 'API Version', value: 'v4', status: 'neutral', icon: <span style={{ fontSize: '16px' }}>🔗</span> },
  ];

  return (
    <div data-testid="factor-model-page" data-ready="true" style={S.page}>
      <div style={{ padding: '12px 16px 0 16px' }}>
        <PageHeader title="Factor Model" subtitle="Multi-factor risk model with factor exposures and decomposition analytics" testId="factor-model-header" />
      </div>
      <div style={{ padding: '0 16px 8px 16px' }}>
        <Tabs items={TABS} activeTab={tab} onTabChange={setTab} testId="factor-model-tabs" />
      </div>
      <div style={S.content}>
        ...
        {tab === 'overview' && (
          <div style={S.gap}>
            <KPIStrip items={kpiItems} variant="hero" testId="factor-model-kpi" />
            <div style={S.grid2}>
              <Panel title="Service Status" variant="elevated" padding="md" testId="factor-model-status"
                status={<StatusBadge variant="success">Operational</StatusBadge>}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  <InfoRow label="Feature" value="Factor Model" />
                  <InfoRow label="Week" value="W32" />
                  <InfoRow label="API Prefix" value="/api/v4/factor-model" />
                  <InfoRow label="Endpoints" value="5" />
                  <InfoRow label="Version" value="v4" />
                </div>
              </Panel>
              <Panel title="Quick Actions" variant="elevated" padding="md" testId="factor-model-actions">
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--ui2-space-2)' }}>
                  <Button variant="primary" fullWidth onClick={fetchData} loading={loading} testId="factor-model-refresh">
                    Refresh Data
                  </Button>
                  <Button variant="secondary" fullWidth testId="factor-model-export">
                    Export Report
                  </Button>
                  <Button variant="ghost" fullWidth testId="factor-model-docs">
                    View Documentation
                  </Button>
                </div>
              </Panel>
        ...
        {tab === 'data' && (
          <div style={S.gap}>
            {loading ? <Skeleton height={300} /> : data.length > 0 ? (
              <Panel title="Factor Model Data" variant="default" padding="none" testId="factor-model-data-panel">
                <DataTable columns={columns} data={data} keyField="id" density="compact" testId="factor-model-table" />
              </Panel>
            ) : (
        ...
        {tab === 'analytics' && (
          <div style={S.gap}>
            <Panel title="Analytics Overview" variant="elevated" padding="md" testId="factor-model-analytics">
              <div style={S.grid3}>
            ...
            <Panel title="Recent Activity" variant="bordered" padding="md" testId="factor-model-activity">
        ...
        {tab === 'config' && (
          <div style={S.gap}>
            <Panel title="Service Configuration" variant="elevated" padding="md" testId="factor-model-config">
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                <InfoRow label="API Endpoint" value="/api/v4/factor-model" />
                <InfoRow label="Week" value="W32" />
            ...
            <Panel title="Endpoints" variant="bordered" padding="md" testId="factor-model-endpoints">
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <div style={S.surface}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>List risk factors</div>
                </div>
                <div style={S.surface}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ui2-text-primary)' }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>Get portfolio factor exposures</div>
                </div>
                <div style={S.surface}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ui2-text-primary)' }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>Get risk decomposition</div>
                </div>
                <div style={S.surface}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ui2-text-primary)' }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>Analyze portfolio factors</div>
                </div>
                <div style={S.surface}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ui2-text-primary)' }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>Get factor covariance matrix</div>
                </div>
              </div>
            </Panel>
          </div>
