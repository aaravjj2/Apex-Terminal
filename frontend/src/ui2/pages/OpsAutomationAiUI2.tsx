...
﻿// OpsAutomationAiUI2 — Bloomberg APEX AI-powered ops automation terminal
  ...
  return (
    <div style={{ background: BG, height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden', fontFamily: MONO, color: TEXT }}>
      <div style={{ borderBottom: `1px solid ${BORDER}`, padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0 }}>
        ...
        <span style={{ fontSize: 10, color: SUBTLE }}>OPS AUTOMATION AI — RUNBOOK GEN + INCIDENT RESPONSE + TASK ENGINE</span>
        {criticalIncidents > 0 && <span style={{ fontSize: 10, color: RED, fontWeight: 700 }}>⚠‘ {criticalIncidents} CRITICAL</span>}
        ...
        {tab === 'runbooks' && (
          <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 4, overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {runbooks.length === 0 && <tr><td colSpan={10} style={{ padding: 24, textAlign: 'center', color: SUBTLE, fontFamily: MONO, fontSize: 11 }}>No runbooks</td></tr>}
                {runbooks.sort((a, b) => b.executionCount - a.executionCount).map((r, i) => (
                  <tr key={i} style={{ opacity: r.status === 'archived' || r.status === 'deprecated' ? 0.5 : 1 }}>
                    ...
                    <Td mono col={SUBTLE}>{r.lastUpdated || '—'}</Td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <tbody>
                ...
                {incidents.sort((a, b) => {
                  ...
                  <tr key={idx} style={{ background: i.severity === 'critical' && i.status !== 'resolved' ? RED + '0a' : 'transparent' }}>
                    ...
                    <Td mono col={SUBTLE}>{i.rootCauseCategory || '—'}</Td>
                    <Td><ConfBar pct={i.aiConfidencePct} /></Td>
                    <Td right mono col={i.mttrMin > 60 ? ORANGE : SUBTLE}>{i.mttrMin > 0 ? i.mttrMin.toFixed(0) : '—'}</Td>
                    <Td right mono col={TEXT}>{i.aiActionsCount}</Td>
                    <Td mono col={BLUE}>{i.runbookId || '—'}</Td>
        ...
        {tab === 'tasks' && (
          <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 4, overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {tasks.length === 0 && <tr><td colSpan={10} style={{ padding: 24, textAlign: 'center', color: SUBTLE, fontFamily: MONO, fontSize: 11 }}>No tasks</td></tr>}
                {tasks.sort((a, b) => a.priority - b.priority).map((t, i) => (
                  <tr key={i} style={{ background: t.status === 'failed' ? RED + '0a' : t.status === 'running' ? AMBER + '07' : 'transparent' }}>
                    ...
                    <Td><span style={{ fontFamily: MONO, fontSize: 9, color: t.aiGenerated ? PURPLE : SUBTLE }}>{t.aiGenerated ? 'âœ“ AI' : '—'}</span></Td>
                    <Td right mono col={SUBTLE}>{t.durationSec > 0 ? t.durationSec.toFixed(0) : '—'}</Td>
                    <Td right mono col={t.retryCount > 2 ? ORANGE : SUBTLE}>{t.retryCount}</Td>
                    <Td mono col={SUBTLE}>{t.scheduledAt}</Td>
                    <Td mono col={SUBTLE}>{t.completedAt || '—'}</Td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {tab === 'decisions' && (
          <div style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 4, overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {decisions.length === 0 && <tr><td colSpan={8} style={{ padding: 24, textAlign: 'center', color: SUBTLE, fontFamily: MONO, fontSize: 11 }}>No decisions</td></tr>}
                {decisions.map((d, i) => (
                  <tr key={i} style={{ background: d.outcome === 'overridden' ? ORANGE + '0a' : d.outcome === 'rejected' ? RED + '0a' : 'transparent' }}>
                    ...
                    <Td mono col={SUBTLE}>{d.overrideReason || '—'}</Td>
