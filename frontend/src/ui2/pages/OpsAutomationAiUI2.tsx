/**
 * OpsAutomationAiUI2 — W98: Ops Automation AI
...
const API = '/api/v4/ops-automation-ai';
...
export function OpsAutomationAiUI2() {
  ...
  const kpiItems: KPIItem[] = [
    ...
    { id: 'week', label: 'Week', value: 'W98', status: 'neutral', icon: <span style={{ fontSize: '16px' }}>🤖</span> },
    { id: 'version', label: 'API Version', value: 'v4', status: 'neutral', icon: <span style={{ fontSize: '16px' }}>🔗</span> },
  ];

  return (
    <div data-testid="ops-automation-ai-page" data-ready="true" style={S.page}>
      <div style={{ padding: '12px 16px 0 16px' }}>
        <PageHeader title="Ops Automation AI" subtitle="AI-powered operational automation with runbook generation and incident response" testId="ops-automation-ai-header" />
      </div>
      <div style={{ padding: '0 16px 8px 16px' }}>
        <Tabs items={TABS} activeTab={tab} onTabChange={setTab} testId="ops-automation-ai-tabs" />
      </div>
      <div style={S.content}>
        ...
        {tab === 'overview' && (
          <div style={S.gap}>
            <KPIStrip items={kpiItems} variant="hero" testId="ops-automation-ai-kpi" />
            <div style={S.grid2}>
              <Panel title="Service Status" variant="elevated" padding="md" testId="ops-automation-ai-status"
                status={<StatusBadge variant="success">Operational</StatusBadge>}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  <InfoRow label="Feature" value="Ops Automation AI" />
                  <InfoRow label="Week" value="W98" />
                  <InfoRow label="API Prefix" value="/api/v4/ops-automation-ai" />
                  <InfoRow label="Endpoints" value="5" />
                  <InfoRow label="Version" value="v4" />
                </div>
              </Panel>
              <Panel title="Quick Actions" variant="elevated" padding="md" testId="ops-automation-ai-actions">
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--ui2-space-2)' }}>
                  <Button variant="primary" fullWidth onClick={fetchData} loading={loading} testId="ops-automation-ai-refresh">
                    Refresh Data
                  </Button>
                  <Button variant="secondary" fullWidth testId="ops-automation-ai-export">
                    Export Report
                  </Button>
                  <Button variant="ghost" fullWidth testId="ops-automation-ai-docs">
                    View Documentation
                  </Button>
                </div>
              </Panel>
        ...
        {tab === 'data' && (
          <div style={S.gap}>
            {loading ? <Skeleton height={300} /> : data.length > 0 ? (
              <Panel title="Ops Automation AI Data" variant="default" padding="none" testId="ops-automation-ai-data-panel">
                <DataTable columns={columns} data={data} keyField="id" density="compact" testId="ops-automation-ai-table" />
              </Panel>
            ) : (
        ...
        {tab === 'analytics' && (
          <div style={S.gap}>
            <Panel title="Analytics Overview" variant="elevated" padding="md" testId="ops-automation-ai-analytics">
              <div style={S.grid3}>
            ...
            <Panel title="Recent Activity" variant="bordered" padding="md" testId="ops-automation-ai-activity">
        ...
        {tab === 'config' && (
          <div style={S.gap}>
            <Panel title="Service Configuration" variant="elevated" padding="md" testId="ops-automation-ai-config">
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                <InfoRow label="API Endpoint" value="/api/v4/ops-automation-ai" />
                <InfoRow label="Week" value="W98" />
            ...
            <Panel title="Endpoints" variant="bordered" padding="md" testId="ops-automation-ai-endpoints">
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <div style={S.surface}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>List generated runbooks</div>
                </div>
                <div style={S.surface}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ui2-text-primary)' }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>Generate runbook from incident</div>
                </div>
                <div style={S.surface}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ui2-text-primary)' }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>List active automations</div>
                </div>
                <div style={S.surface}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ui2-text-primary)' }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>Execute automation</div>
                </div>
                <div style={S.surface}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ui2-text-primary)' }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--ui2-text-tertiary)', marginTop: '2px' }}>Get AI operation suggestions</div>
                </div>
              </div>
            </Panel>
          </div>
