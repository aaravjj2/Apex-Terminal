...
test.describe('v1.56 — Market Tape', () => {
  ...
  test('market tape prices match watchlist/demo source', async ({ page }) => {
    await page.goto(`${BASE}/ui2/dashboard`);
    // allow demo stream to start
    await page.waitForTimeout(2000);

    // Read MarketTape AAPL price (span[1] is the price element)
    const tapeTick = page.getByTestId('ui2-market-tape-tick-AAPL');
    const tapePriceText = await tapeTick.locator('span').nth(1).textContent();
    const tapePrice = tapePriceText?.trim();

    // Read latest deterministic simulator price exposed in-DOM for E2E checks
    const simPriceText = await page.getByTestId('ui2-stream-latest-AAPL').textContent();
    const simPrice = simPriceText?.trim();

    expect(tapePrice).toBeDefined();
    expect(simPrice).toBeDefined();
    // MarketTape must reflect the simulator's latest tick
    expect(tapePrice).toBe(simPrice);
  });
});
